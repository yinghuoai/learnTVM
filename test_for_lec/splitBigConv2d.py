...
# 尝试TensorIR实现，这是一个完整的conv的实现：1*1*128*128
@tvm.script.ir_module
class MyConv:
    @T.prim_func
    def conv(A: T.Buffer[ (1, 1, 128, 128), "int64"],
            B: T.Buffer[ (2, 1, 3, 3), "int64"],
            C: T.Buffer[ (1, 2, 126, 126), "int64"]):
        T.func_attr({"global_symbol":"conv", "tir.noalias":True})
        for b, k, i, j in T.grid(1, 2, 126, 126):
            with T.block("C"):
                vb = T.axis.spatial(1, b)
                vk = T.axis.spatial(2, k)
                vi = T.axis.spatial(126, i)
                vj = T.axis.spatial(126, j)
...
class MyThreadSplit4Conv():
    ...
    def conv(A: T.Buffer[ (1, 1, 128, 128), "int64"],
        ...
        T.func_attr({"global_symbol":"conv", "tir.noalias":True})
        # tmp 
        Y = T.alloc_buffer(( 1, 1, 65, 65), dtype="int64")
        # r_d = T.var("int32")
        # c_d = T.var("int32")
        ...
        # for num in T.grid(4):
        for tt, ttt, row, col in T.grid(1, 1, 65, 65):
            with T.block("Y"):
                dd = T.axis.spatial(1, tt)
                ddd = T.axis.spatial(1, ttt)
                rr = T.axis.spatial(65, row)
                cc = T.axis.spatial(65, col)
                with T.init():
        ...
        for b, k, i, j in T.grid(1, 2, 63, 63):
            with T.block("C"):
                vb = T.axis.spatial(1, b)
                vk = T.axis.spatial(2, k)
                vi = T.axis.spatial(63, i)
                vj = T.axis.spatial(63, j)
                for di, dj in T.grid(3,3):    
                    with T.init():
    ...
    def conv(A: T.Buffer[ (1, 1, 128, 128), "int64"],
        ...
        for num in T.grid(4):
            ...
            if (r_d[0,0]== 0 and r_d[0,1]==0):
                r_d[0,1] = 63
            elif (r_d[0,0]==0 and r_d[0,1]==63):
                r_d[0,0] = 63
                r_d[0,1] = 0
            elif (r_d[0,0]== 63 and r_d[0,1]==0):
                r_d[0,1]= 63
